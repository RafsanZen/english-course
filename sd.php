<?php
    ...
    $sql_video = "SELECT video.url, video.deskripsi, pendidikan.pendidikan 
                    ...
                    WHERE pendidikan.pendidikan = 'SD'
                    ORDER BY video.created_at DESC";
    $result_video_sd = $conn->query($sql_video);
?>
...
<section class="py-6">
    ...
    <div class="container mt-5">
        <h4>Pembelajaran Bahasa Inggris Untuk Sekolah Dasar (SD)</h4>
        ...
        <div class="col-4 mt-3">
            ...
            <ul class="list-group mt-3">
                <?php if($result_video_sd->num_rows > 0) { ?>
                    <?php while($row = $result_video_sd->fetch_assoc()) { ?>
                        <a href="customer.php?halaman=sd&url=<?= $row['url'] ?>" class="text-dark"><li class="list-group-item"><?= $row['deskripsi'] ?></li></a>
                    <?php } ?>
                <?php } ?>
            </ul>
        </div>
