<?php
    ...
    $sql_video = "SELECT video.url, video.deskripsi, pendidikan.pendidikan 
                    ...
                    WHERE pendidikan.pendidikan = 'SMA'
...
<section class="py-6">
    ...
    <div class="container mt-5">
        <h4>Pembelajaran Bahasa Inggris Untuk Sekolah Menengah Atas (SMA)</h4>
            ...
            <ul class="list-group mt-3">
                <?php if($result_video->num_rows > 0) { ?>
                    <?php while($row = $result_video->fetch_assoc()) { ?>
                        <a href="customer.php?halaman=sma&url=<?= $row['url'] ?>" class="text-dark"><li class="list-group-item"><?= $row['deskripsi'] ?></li></a>
                    <?php } ?>
                <?php } ?>
            </ul>
        </div>
